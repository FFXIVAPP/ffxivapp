...
namespace FFXIVAPP.ViewModels
{
    public class SettingsVM
    {
        ...
        public static void SaveCharacter()
        {
            try
            {
                var first = SettingsVCharacter.View.FirstName.Text;
                var last = SettingsVCharacter.View.LastName.Text;
        ...
        {
            if (SettingsVLogColor.View.Colors.SelectedItems.Count > 0)
            {
                var t = SettingsVLogColor.View.Colors.SelectedItem.ToString().Split(',');
                _key = t[0].Trim().Replace("[", "");
                _colorcode = Constants.XColor[_key][0];
                SettingsVLogColor.View.TKey.Text = _key;
                SettingsVLogColor.View.TValue.Text = _colorcode;
            }
        }
        ...
        {
            _key = SettingsVLogColor.View.TKey.Text;
            _colorcode = SettingsVLogColor.View.TValue.Text;
            if (!String.IsNullOrEmpty(_key))
            {
                if (Regex.IsMatch(_colorcode, "^[A-F0-9]{6,6}$"))
                {
                    Constants.XColor[_key][0] = _colorcode;
                    SettingsVLogColor.View.Colors.Items.Refresh();
                }
                else
                {
                    string title, message;
