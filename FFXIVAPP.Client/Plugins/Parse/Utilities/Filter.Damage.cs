...
namespace FFXIVAPP.Client.Plugins.Parse.Utilities
{
    ...
    {
        private static void ProcessDamage(Models.Events.Event e, Expressions exp)
        {
            var line = new Line
            {
                RawLine = e.RawLine
        ...
        private static void UpdateDamagePlayer(Match damage, Line line, Expressions exp, bool isParty = true)
        {
            ...
            try
            {
                ...
                if (isParty)
                {
                    ...
                    if (!_autoAction && (line.IsEmpty() || (!_isMulti && _lastEventParty.Type != EventType.Actions)) && !_lastActionIsAttack)
                    {
                        ClearLast(true);
                        return;
                    }
                ...
                {
                    if (!_autoAction && (line.IsEmpty() || (!_isMulti && _lastEventPlayer.Type != EventType.Actions)) && !_lastActionIsAttack)
                    {
                        return;
                    }
                }
        ...
        private static void UpdateDamageMonster(Match damage, Line line, Expressions exp, bool isParty = true)
        {
            ...
            try
            {
                ...
                if (isParty)
                {
                    ...
                    if (!_autoAction)
                    {
                        if (line.IsEmpty() || (!_isMulti && _lastEventParty.Type != EventType.Actions) && !_lastActionIsAttack)
                        {
                            ClearLast(true);
                            return;
                        }
                ...
                else
                {
                    ...
                    if (!_autoAction)
                    {
                        if (line.IsEmpty() || (!_isMulti && _lastEventPlayer.Type != EventType.Actions) && !_lastActionIsAttack)
                        {
                            return;
                        }
                    }
