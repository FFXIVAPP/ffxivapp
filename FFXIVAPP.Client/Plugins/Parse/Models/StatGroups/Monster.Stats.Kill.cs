...
namespace FFXIVAPP.Client.Plugins.Parse.Models.StatGroups
{
    public partial class Monster
    {
        ...
        public void SetKill(Fight fight)
        {
            ...
            {
                Logging.Log(LogManager.GetCurrentClassLogger(), String.Format("KillEvent : Got request to add kill stats for {0}, but my name is {1}!", fight.MonsterName, Name));
                return;
            }
            if (fight.MonsterName == "")
            {
                Logging.Log(LogManager.GetCurrentClassLogger(), String.Format("KillEvent : Got request to add kill stats for {0}, but no name!", fight.MonsterName));
                return;
            }
