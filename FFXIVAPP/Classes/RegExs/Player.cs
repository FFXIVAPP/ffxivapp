...
namespace FFXIVAPP.Classes.RegExs
{
    public static class Player
    {
        ...
        public static Regex ParryFr = new Regex(@"^(L\w |([LEAD' ]+))?(?<source>[\w\s'-]+) utilise( une?)? (?<action>[\w\s'-]+) sur (?<target>\w+\s\w+)( \((?<direction>.+)\))? mais.+arrive à (?<parry>parer)\.(\w+ \w+ (?<partial>ne subit aucun).+!|\w+ \w+ subit (?<amount>\d+) points?.+\(mit.+\)\.)$", Shared.DefaultOptions);
        ...
        public static Regex MultiFr = new Regex(@"(?!.+util)^(?<crit>Coup critique! )?(((L\w+ |(a|à)(\w+)? )?(L\')?(?<target>.+))) subit (?<amount>\d+) points?.+\. ?$", Shared.DefaultOptions);
