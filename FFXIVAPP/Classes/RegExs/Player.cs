...
namespace FFXIVAPP.Classes.RegExs
{
    public static class Player
    {
        ...
        public static readonly Regex BlockFr = new Regex(@"^(l|L)?\w+ (?<source>.+) util.+un\w+ (?<action>.+) sur (?<player>\w+\s\w+).+mais.+bouclier\.(.+par.+(?<block>coup)!$|.+(?<block>subit) (?<amount>\d+) points de d.+\.$)", Shared.DefaultOptions);
        ...
        public static readonly Regex ParryFr = new Regex(@"^\.$", Shared.DefaultOptions);
