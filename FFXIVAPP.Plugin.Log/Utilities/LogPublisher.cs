...
namespace FFXIVAPP.Plugin.Log.Utilities
{
    public static class LogPublisher
    {
        public static void Process(ChatEntry chatEntry)
        {
            ...
            {
                // handle tabs
                if (CheckMode(chatEntry.Code, Constants.ChatPublic))
                {
            ...
            try
            {
                ...
                {
                    if (CheckMode(chatEntry.Code, Constants.ChatSay) && Settings.Default.TranslateSay)
                    {
                        GoogleTranslate.RetreiveLang(rawLine, chatEntry.JP);
                    }
                    if (CheckMode(chatEntry.Code, Constants.ChatTell) && Settings.Default.TranslateTell)
                    {
                        GoogleTranslate.RetreiveLang(rawLine, chatEntry.JP);
                    }
                    if (CheckMode(chatEntry.Code, Constants.ChatParty) && Settings.Default.TranslateParty)
                    {
                        GoogleTranslate.RetreiveLang(rawLine, chatEntry.JP);
                    }
                    if (CheckMode(chatEntry.Code, Constants.ChatShout) && Settings.Default.TranslateShout)
                    {
                        GoogleTranslate.RetreiveLang(rawLine, chatEntry.JP);
                    }
                    if (CheckMode(chatEntry.Code, Constants.ChatYell) && Settings.Default.TranslateYell)
                    {
                        GoogleTranslate.RetreiveLang(rawLine, chatEntry.JP);
                    }
                    if (CheckMode(chatEntry.Code, Constants.ChatLS) && Settings.Default.TranslateLS)
                    {
                        GoogleTranslate.RetreiveLang(rawLine, chatEntry.JP);
                    }
                    if (CheckMode(chatEntry.Code, Constants.ChatFC) && Settings.Default.TranslateFC)
                    {
                        GoogleTranslate.RetreiveLang(rawLine, chatEntry.JP);
                    }
                }
